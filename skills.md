...
entity and `FullAWSAccess` uses one. The `require-tags-product` policy denies resource creation unless these tags are present:
...
## AWS tags applied
On the SCP module's own resources: `Project`, `ManagedBy`, `Repository`, `Team`, `CostCenter` (see `locals.full_tags`). This is separate from `Environment`/`Team`/`ManagedBy` — the tags the `require-tags-product` *policy* enforces on other org resources (EC2, EKS, RDS, etc.); there's no `environment` input variable, so the module's own resources are never tagged `Environment`.
